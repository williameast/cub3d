...
/*   Updated: 2025/11/14 12:11:31 by weast            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_invalid(int Y, int X, t_map *map, char **visited)
{
	if ((map->grid[Y][X] == '1') || (visited[Y][X]))
		return (0);
	visited[Y][X] = '1';
	if ((is_edge(Y, X, map) || !is_closed(Y, X, map, visited)))
		return (1);
	return (0);
}
...
int	is_closed(int Y, int X, t_map *map, char **visited)
{
	if ((is_invalid(Y - 1, X, map, visited)) || (is_invalid(Y + 1, X, map,
				visited)) || (is_invalid(Y, X - 1, map, visited))
